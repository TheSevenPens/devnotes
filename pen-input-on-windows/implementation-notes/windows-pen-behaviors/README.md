...
{% hint style="info" %}
**Before you try to disable anything, check the baseline.** Whether Windows even
_produces_ these behaviors depends on system-wide settings and on the input device
— an indirect pen tablet often doesn't drive the shell feedback at all. See
[Windows pen & touch settings](windows-pen-touch-settings.md). Working, toggleable
demo apps for every behavior in this section live in the
[sample apps](sample-apps-to-build.md).
{% endhint %}
